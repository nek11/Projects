...
class DISN(nn.Module):
    def __init__(
        self,
        latent_size,
        perceptual_latent_in_size=()
    ):
        super(DISN, self).__init__()
        perceptual_size = np.sum(perceptual_latent_in_size)
        ...
        global_net.append(nn.ReLU())
        global_net.append(nn.Linear(256, 512))
        global_net.append(nn.BatchNorm1d(num_features=512))
        global_net.append(nn.ReLU())
        self.global_net = nn.Sequential(*global_net)

        global_concat_net = []
        global_concat_net.append(nn.Linear(512+latent_size, 512))
        global_concat_net.append(nn.BatchNorm1d(num_features=512))
        global_concat_net.append(nn.ReLU())
        global_concat_net.append(nn.Linear(512, 256))
...
class DeepTrans(nn.Module):
    def __init__(
        ...
        # Simple MLP for finding translational vector, input = latent_vector
        translation_layers = []
        translation_layers.append(nn.Linear(latent_size, 512))
        translation_layers.append(nn.ReLU())
        translation_layers.append(nn.Linear(512, 512))
        translation_layers.append(nn.ReLU())
        translation_layers.append(nn.Linear(512, 1))
        self.translation_layers = nn.Sequential(*translation_layers)

    def forward(self, trans_z):

        trans_z_proc = self.translation_layers(trans_z)
