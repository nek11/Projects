...
os.environ["CUDA_VISIBLE_DEVICES"]="0"         # Here choose which GPU to use
...
def main_function(experiment_directory, continue_from,  iterations, marching_cubes_resolution, regularize):
    ...
    sdf_dataset_test = lib.data.RGBA2SDF_reconstruct(
    ...
    #Replace here with for image in images (for images of real cars) and take off parts that compute mesh with vertices, etc
    for sdf_data, image, intrinsic, extrinsic, name, vertices, faces_cam in sdf_loader_test:
        print(image.shape)
        ...
        # Change type to cuda before giving as arguments to Meshes, these are vertices and faces loaded from rgba2sdf_reconstruct
        vertices = vertices.cuda()
        faces_cam = faces_cam.cuda()

        meshes_gt = Meshes(vertices, faces_cam)
